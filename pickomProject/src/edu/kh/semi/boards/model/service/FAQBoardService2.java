...
import edu.kh.semi.boards.model.dao.FAQBoardDAO2;

import edu.kh.semi.boards.model.vo.FAQAttachment;
import edu.kh.semi.boards.model.vo.FAQBoard;


public class FAQBoardService2 {
	
	private FAQBoardDAO2 dao = new FAQBoardDAO2();
	 ...
	 * @throws Exception
	 */
	public int insertBoard(FAQBoard board, List<FAQAttachment> atList) throws Exception{
		...
		if(boardNo > 0) {
			...
			// textarea의 개행문자 : \r\n
			// div의 개행문자 : <br>
			String boardContent = board.getFaqContent();
...
//					board.getBoardContent().replaceAll("\r\n", "<br>"); -> 더 짧은 코드!
	...
	// 크로스 사이트 스크립트 방지 메소드
		...
		public int updateBoard(FAQBoard board, List<FAQAttachment> atList) throws Exception {
			Connection conn = getConnection();
			...
			board.setFaqContent(replaceParameter(board.getFaqContent()));
			board.setFaqTitle(replaceParameter(board.getFaqTitle())); // input type="text"는 개행문자가 못 들어옴
			
			board.setFaqContent(board.getFaqContent().replaceAll("\r\n", "<br>"));
